from assault_plans import (
    make_test_assault_plan, make_easy_assault_plan, make_normal_assault_plan,
    make_hard_assault_plan, make_extra_hard_assault_plan
)
from places import Skynet, DragonColony
from layouts import dry_layout, wet_layout, dragon_types


def start_with_strategy(args, strategy):
    """Reads command-line arguments and starts a game with those options."""
    import argparse
    parser = argparse.ArgumentParser(description="Play Dragons Vs Terminators")
    ...
    skynet = Skynet(assault_plan)
    dimensions = (num_tunnels, tunnel_length)
    return DragonColony(strategy, skynet, dragon_types(), layout, dimensions,
                     food).simulate()
